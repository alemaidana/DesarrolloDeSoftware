...
class Fecha{
	...
	public:
		...
		//declaramos los getters de las variables
		int getDia();
		int getMes();
		int getAnio();
...
bool Fecha::validarDia(){
		...
		if ((dia <= 31) && (dia > 0)){
			...
			//si el dia no es valido se le preguntara al usuario
...
string Fecha::dameFecha(){
	...
	string dia_final = dr;
	string mes_final = mr;
	string anio_final = ar;
	
	
	string total = dia_final + "/" + mes_final + "/" + anio_final;
